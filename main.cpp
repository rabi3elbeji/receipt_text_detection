...
int main()
{
    ...
    int inpWidth = 512;
    int inpHeight = 512;
    String model = "../model/receipt_model.pb";
    String modeltxt = "../model/graph.pb";
    ...
    // input file path
    std::string filePath  = "../img_1.jpg";


    static const std::string kWinName = "EAST: An Efficient and Accurate Scene Text Detector";

    std::vector<Mat> output;
    std::vector<String> inutLayers(1);
    inutLayers[0]="input_images";
    ...
    for (size_t i = 0; i < indices.size(); ++i)
    {
        ...
        for (int j = 0; j < 4; ++j)
            line(image, vertices[j], vertices[(j + 1) % 4], Scalar(0, 255, 0), 6, LINE_AA);
    }
